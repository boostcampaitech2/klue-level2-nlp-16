...
_criterion_entrypoints = {
    ...
    'f1': F1Loss,
    'dice': SelfAdjDiceLoss
}


def criterion_entrypoint(loss_name):
    return _criterion_entrypoints[loss_name]
...
def create_criterion(loss_name, **kwargs):
    if is_criterion(loss_name):
        create_fn = criterion_entrypoint(loss_name)
        
        if (loss_name == 'cross_entropy' or loss_name == 'focal') and 'classes' in kwargs.keys():
            del(kwargs['classes'])
